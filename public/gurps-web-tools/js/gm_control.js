...
function gm_control_start_combat() {
  ...
  if (gm_control_sheet.length > 0) $(".js-turn-controls").slideDown();
  else
    create_alert(
      "It's probably a good idea to have participants in the combat before you start combat.",
      "warning",
    );
}
...
function gm_control_update_turn_box() {
  html =
    "<fieldset><legend>Combat Turn Control - Turn #" +
    gm_control_current_turn +
    "</legend>";
  if (gm_control_current_combatatant > 0) {
    html +=
      '<a href="#" class="js-gm-control-go-to-beginning-turn"><span class="glyphicon glyphicon-fast-backward" title="Go to start of turn"></span></a>';
    html +=
      '<a href="#" class="js-gm-control-go-to-previous-turn"><span class="glyphicon glyphicon-backward" title="Start back a turn"></span></a>';
  } else {
    html +=
      '<span class="glyphicon glyphicon glyphicon-fast-backward" title="Start Combat Session"></span>';
    html +=
      '<span class="glyphicon glyphicon glyphicon-backward" title="Start Combat Session"></span>';
  }
  html +=
    '<a href="#" class="js-gm-control-stop-combat"><span class="glyphicon glyphicon glyphicon-stop" title="Stop Combat Session"></span></a>';
  html +=
    '<a href="#" class="js-gm-control-go-to-next-turn"><span class="glyphicon glyphicon glyphicon-forward" title="Go to next turn"></span></a>';
  html += "</fieldset>";
  ...
  $(".js-gm-control-go-to-previous-turn").click(function (event) {
    ...
    if (gm_control_current_combatatant < 0) gm_control_current_combatatant = 0;
    gm_control_update_turn_box();
  });
...
function gm_control_export_json(selected_only) {
  ...
  for (count = 0; count < gm_control_sheet.length; count++) {
    export_item = {
      ...
      melee_weapon_attack_skill:
        gm_control_sheet[count].get_melee_weapon_attack_skill(),
      ranged_weapon_attack_skill:
        gm_control_sheet[count].get_ranged_weapon_attack_skill(),
      brawling_skill: gm_control_sheet[count].get_brawling_skill(),

      weapon: {
        ready: gm_control_sheet[count].get_weapon("ready"),
        time_to_ready: gm_control_sheet[count].get_weapon("time_to_ready"),
        time_between_shots:
          gm_control_sheet[count].get_weapon("time_between_shots"),
        time_to_reload: gm_control_sheet[count].get_weapon("time_to_reload"),
        shots_current: gm_control_sheet[count].get_weapon("shots_current"),
        shots_max: gm_control_sheet[count].get_weapon("shots_max"),
      },
  ...
  try {
    ...
    if (overwrite) {
      for (var i = gm_control_sheet.length - 1; i >= 0; i--) {
        ...
        if (!player_name) gm_control_sheet.splice(i, 1);
      }
    }
    ...
    if (typeof import_object == "object") {
      ...
      for (
        import_count = 0;
        import_count < import_object.length;
        import_count++
      ) {
    ...
    create_alert(
      "Could not import JSON. Please check the formatting!",
      "danger",
    );
  }
}
...
function gm_control_import_object(importing_object) {
  ...
  if (typeof importing_object.secondary.fatigue != "undefined")
    return_value.set_secondary(
      "fatigue",
      importing_object.secondary.fatigue / 1,
    );
  if (typeof importing_object.secondary.curr_fatigue != "undefined")
    return_value.set_secondary(
      "curr_fatigue",
      importing_object.secondary.curr_fatigue / 1,
    );
  ...
  if (typeof importing_object.secondary.curr_hp != "undefined")
    return_value.set_secondary(
      "curr_hp",
      importing_object.secondary.curr_hp / 1,
    );

  if (typeof importing_object.secondary.reaction != "undefined")
    return_value.set_secondary(
      "reaction",
      importing_object.secondary.reaction / 1,
    );
  ...
  if (typeof importing_object.melee_weapon_attack_skill != "undefined")
    return_value.set_melee_weapon_attack_skill(
      importing_object.melee_weapon_attack_skill,
    );
  if (typeof importing_object.ranged_weapon_attack_skill != "undefined")
    return_value.set_ranged_weapon_attack_skill(
      importing_object.ranged_weapon_attack_skill,
    );
  if (typeof importing_object.brawling_skill != "undefined")
    return_value.set_brawling_skill(importing_object.brawling_skill);

  if (typeof importing_object.weapon != "undefined") {
    const weapon_fields = [
      "ready",
      "time_to_ready",
      "time_between_shots",
    ...
    for (let field of weapon_fields) {
      if (typeof importing_object.weapon[field] !== "undefined") {
        const value = importing_object.weapon[field];
        return_value.set_weapon(
          field,
...
function gm_control_display_sheet() {
  ...
  if (gm_control_sheet.length > 0) {
    for (count = 0; count < gm_control_sheet.length; count++) {
      ...
      if (
        gm_control_current_turn > 0 &&
        gm_control_current_combatatant == count
      )
      ...
      is_pc = "is_pc='0'";
      if (player_name) is_pc = "is_pc='1'";

      html +=
        '<td class="text-right"><span class="glyphicon glyphicon-move drag-select"></span><input ' +
        is_pc +
        " " +
        checked +
        'type="checkbox" ref="' +
        count +
        '" class="js-select-check" /></td>';
      html += "<td>";
      html +=
        '<a href="#" ref="' +
        count +
        '" title="View This Entry" class="js-gm-control-line-view"><span class="glyphicon glyphicon-eye-open"></span></a> ';
      is_hidden = "style='display: none'";
      if (gm_control_sheet[count].shock_amount) is_hidden = "";
      html +=
        "<span class='shock-damage js-shock-damage-" +
        count +
        "' " +
        is_hidden +
        " title='This character is in shock!'>-" +
        gm_control_sheet[count].shock_amount +
        "</span>";
      html += gm_control_sheet[count].get_name();

      if (player_name) html += " (" + player_name + ")";
      html +=
        '<div class="js-mobile-details js-mobile-details-' +
        count +
        '" style="display:none">';
      html += "<h5>Attributes</h5>";
      html +=
        "<div><strong>ST</strong>: " +
        gm_control_sheet[count].get_attribute("st") +
        " <strong>DX</strong>: " +
        gm_control_sheet[count].get_attribute("dx") +
        " <strong>IQ</strong>: " +
        gm_control_sheet[count].get_attribute("iq") +
        " <strong>HT</strong>: " +
        gm_control_sheet[count].get_attribute("ht") +
        "</div>";
      html +=
        "<div>Reaction: " +
        gm_control_sheet[count].get_secondary("reaction") +
        "</div>";
      html +=
        "<div>Will/Perception: " +
        gm_control_sheet[count].get_secondary("will") +
        "/" +
        gm_control_sheet[count].get_secondary("per") +
        "</div>";
      html += '<div class="text-right">';
      html +=
        ' <a href="#" ref="' +
        count +
        '" title="Edit This Entry" class="js-gm-control-line-edit"><span class="glyphicon glyphicon-edit"></span></a> ';
      html +=
        ' <a href="#" ref="' +
        count +
      ...
      html +=
        ' <a href="#" ref="' +
        count +
        '" title="Remove This Entry" class="js-gm-control-line-remove"><span class="glyphicon glyphicon-trash"></span></a> ';
      html += "</div>";
      html += "</div>";
      html += "</td>";
      ...
      html +=
        ...
        gm_control_sheet[count].get_secondary("speed");
      if (gm_control_sheet[count].random_roll)
        html +=
          ' <sub title="This is the random roll for ties">' +
          gm_control_sheet[count].random_roll +
          "</sub>";
      html += " </td>";
      html +=
        '<td class="text-center">' +
        gm_control_sheet[count].get_secondary("move") +
        "</td>";
      html +=
        '<td class="text-center">' +
        gm_control_sheet[count].get_defense("dodge") +
        " / " +
        gm_control_sheet[count].get_defense("block") +
        " / " +
        gm_control_sheet[count].get_defense("parry") +
        "</td>";
      html +=
        '<td class="text-center">' +
        gm_control_sheet[count].get_defense("dr") +
        "</td>";
      html += '<td class="text-center">';
      html +=
        '<a href="#" ref="' +
        count +
        '" title="Apply Damage" class="js-gm-control-damage-modify js-gm-control-damage-modify-' +
        count +
        '">' +
        gm_control_sheet[count].get_secondary("curr_hp") +
        "</a> / " +
        gm_control_sheet[count].get_secondary("hp");
      html += '<div class="js-gm-control-damage-modify-dropdown">';
      html +=
        '<a href="#" ref="' +
        count +
        '" class="js-gm-control-damage-modify-zero"><span class="glyphicon glyphicon-circle-arrow-down"></span></a>';
      html +=
        '<a href="#" ref="' +
        count +
        '" class="js-gm-control-damage-modify-minus"><span class="glyphicon glyphicon-minus"></span></a>';
      html +=
        '<a href="#" ref="' +
        count +
        '" class="js-gm-control-damage-modify-ok"><span class="glyphicon glyphicon-ok"></span></a>';
      html +=
        '<a href="#" ref="' +
        count +
        '" class="js-gm-control-damage-modify-plus"><span class="glyphicon glyphicon-plus"></span></a>';
      html +=
        '<a href="#" ref="' +
        count +
        '" class="js-gm-control-damage-modify-max"><span class="glyphicon glyphicon-circle-arrow-up"></span></a>';
      html += "</div></td>";
      html += '<td class="text-center">';
      html +=
        '<a href="#" ref="' +
        count +
        '" title="Apply Fatigue" class="js-gm-control-fatigue-modify js-gm-control-fatigue-modify-' +
        count +
        '">' +
        gm_control_sheet[count].get_secondary("curr_fatigue") +
        "</a> / " +
        gm_control_sheet[count].get_secondary("fatigue");

      html += '<div class="js-gm-control-fatigue-modify-dropdown">';
      html +=
        '<a href="#" ref="' +
        count +
        '" class="js-gm-control-fatigue-modify-zero"><span class="glyphicon glyphicon-circle-arrow-down"></span></a>';
      html +=
        '<a href="#" ref="' +
        count +
        '" class="js-gm-control-fatigue-modify-minus"><span class="glyphicon glyphicon-minus"></span></a>';
      html +=
        '<a href="#" ref="' +
        count +
        '" class="js-gm-control-fatigue-modify-ok"><span class="glyphicon glyphicon-ok"></span></a>';
      html +=
        '<a href="#" ref="' +
        count +
        '" class="js-gm-control-fatigue-modify-plus"><span class="glyphicon glyphicon-plus"></span></a>';
      html +=
        '<a href="#" ref="' +
        count +
        '" class="js-gm-control-fatigue-modify-max"><span class="glyphicon glyphicon-circle-arrow-up"></span></a>';
      html += "</div></td>";
      ...
      html +=
        ...
        gm_control_sheet[count].get_ranged_weapon_attack_skill() +
        " / " +
        gm_control_sheet[count].get_brawling_skill() +
        "</td>";

      html += '<td class="text-center">';
      html +=
        '<div><input type="checkbox" class="js-weapon-ready-toggle" data-field="weapon_ready" data-index="' +
        count +
        '" ' +
        (gm_control_sheet[count].get_weapon("ready") ? "checked" : "") +
        " /></div>";

      html +=
        '<div class="row">' +
        '<span class="js-editable js-weapon-time-edit" data-field="weapon_time_to_ready" data-index="' +
        count +
        '">' +
        gm_control_sheet[count].get_weapon("time_to_ready") +
        "</span>" +
        " / " +
        '<span class="js-editable js-weapon-time-edit" data-field="weapon_time_between_shots" data-index="' +
        count +
        '">' +
        gm_control_sheet[count].get_weapon("time_between_shots") +
        "</span>" +
        " / " +
        '<span class="js-editable js-weapon-time-edit" data-field="weapon_time_to_reload" data-index="' +
        count +
        '">' +
        gm_control_sheet[count].get_weapon("time_to_reload") +
        "</span>" +
        "</div>";
      ...
      html +=
        '<div class="row">' +
        '<span class="js-editable js-weapon-shots-edit" data-field="weapon_shots_current" data-index="' +
        count +
        '">' +
        gm_control_sheet[count].get_weapon("shots_current") +
        "</span>" +
        " / " +
        '<span class="js-editable js-weapon-shots-edit" data-field="weapon_shots_max" data-index="' +
        count +
        '">' +
        gm_control_sheet[count].get_weapon("shots_max") +
        "</span>" +
        "</div>";
      ...
      html +=
        '<td class="text-center js-editable js-notes-edit" data-field="notes" data-index="' +
        count +
        '">' +
        gm_control_sheet[count].get_notes() +
        "</td>";
      html += "</div>";
    ...
    html +=
      "<tr><td colspan='15'>There are no items in your control sheet</td></tr>";
  }
  ...
  if (activate_sort_table) {
    ...
    $(".sorted_table").sortable({
      ...
      onDrop: function (item, container, _super) {
        ...
        for (
          select_count = 0;
          select_count < gm_control_sheet_currently_selected.length;
          select_count++
        ) {
          if (
            gm_control_sheet_currently_selected[select_count] / 1 ==
            oldIndex / 1
          ) {
...
function gm_control_propogate_damage_form(character) {
  ...
  if (character) {
    ...
    $(".js-char-entry-field-curr_fatigue").val(
      character.get_secondary("curr_fatigue"),
    );
...
function gm_control_init_entry_form(character) {
  ...
  if (character) {
    ...
    $(".js-char-field-curr_fatigue").val(
      character.get_secondary("curr_fatigue"),
    );
    ...
    $(".js-char-field-melee_weapon_attack_skill").val(
      character.get_melee_weapon_attack_skill(),
    );
    $(".js-char-field-ranged_weapon_attack_skill").val(
      character.get_ranged_weapon_attack_skill(),
    );
    $(".js-char-field-brawling_skill").val(character.get_brawling_skill());

    $(".js-char-field-weapon-ready").prop(
      "checked",
      character.get_weapon("ready"),
    );
    $(".js-char-field-weapon-time-to-ready").val(
      character.get_weapon("time_to_ready"),
    );
    $(".js-char-field-weapon-time-to-reload").val(
      character.get_weapon("time_to_reload"),
    );
    $(".js-char-field-weapon-time-between-shots").val(
      character.get_weapon("time_between_shots"),
    );
    $(".js-char-field-weapon-shots-max").val(character.get_weapon("shots_max"));
    $(".js-char-field-weapon-shots-current").val(
      character.get_weapon("shots_current"),
    );
    ...
    $(".js-char-field-weapon-ready").prop("checked", false);
    $(".js-char-field-weapon-time-to-ready").val("1");
...
function gm_control_update_edit_char(changed_name) {
  ...
  if (changed_name == "ht" || changed_name == "dx") {
    new_speed =
      ($(".js-char-field-ht").val() / 1 + $(".js-char-field-dx").val() / 1) / 4;
    $(".js-char-field-speed").val(new_speed);
    $(".js-char-field-move").val(
      Math.floor($(".js-char-field-speed").val() / 1),
    );
...
function gm_control_assign_data_to_char(character) {
  ...
  character.set_secondary(
    "curr_fatigue",
    $(".js-char-field-curr_fatigue").val() / 1,
  );

  character.set_melee_weapon_attack_skill(
    $(".js-char-field-melee_weapon_attack_skill").val(),
  );
  character.set_ranged_weapon_attack_skill(
    $(".js-char-field-ranged_weapon_attack_skill").val(),
  );
  ...
  character.set_weapon(
    "ready",
    $(".js-char-field-weapon-ready").is(":checked"),
  );
  character.set_weapon(
    "time_to_ready",
    $(".js-char-field-weapon-time-to-ready").val() / 1,
...
function gm_control_show_edit_line_dialog(character, index) {
  ...
  $(".js-gm-control-line-dialog-action-button").on("click", function (event) {
    ...
    gm_control_sheet[gm_control_currently_editing] =
      gm_control_assign_data_to_char(
        gm_control_sheet[gm_control_currently_editing],
      );
...
function gm_control_show_edit_damage_dialog(character, index) {
  ...
  for (damagecount = 1; damagecount < 21; damagecount++)
    options_html +=
      '<option value="' + damagecount + '">' + damagecount + " points</option>";

  $(".js-applied-damage-field").html(options_html);
  $(".js-area-add-more").hide();
  gm_control_currently_editing = index;
  gm_control_propogate_damage_form(
    gm_control_sheet[gm_control_currently_editing],
  );
  ...
  $(".js-gm-control-damage-dialog-action-button").on("click", function (event) {
    ...
    document.getSelection().removeAllRanges();
    // Update data to exiting character in gm_control_sheet
    gm_control_apply_damage(
      gm_control_currently_editing,
      $(".js-applied-damage-field").val(),
    );
    //		console.log( gm_control_sheet[gm_control_currently_editing] );
    // Refresh Sheet
    gm_control_display_sheet();
...
function gm_control_show_edit_fatigue_dialog(character, index) {
  gm_control_init_entry_form(character);
  $(".js-gm-control-fatigue-dialog-action-button")
    .val("Save")
    .button("refresh");
  ...
  $(".js-gm-control-fatigue-dialog-action-button").on(
    "click",
    function (event) {
      event.preventDefault();
      document.getSelection().removeAllRanges();
      // Update data to exiting character in gm_control_sheet
      gm_control_apply_fatigue(
        gm_control_currently_editing,
        $(".js-applied-fatigue-field").val(),
      );
      //		console.log( gm_control_sheet[gm_control_currently_editing] );
      // Refresh Sheet
      gm_control_display_sheet();
...
function gm_control_refresh_events() {
  ...
  $(".js-gm-control-new").click(function (event) {
    ...
    if (
      confirm(
        "This will clear out ALL items (both PCs and NPCs) in your current control sheet. Are you sure you want to do this?",
      )
    )
  ...
  $(".js-gm-control-save").click(function (event) {
    ...
    else number_items = gm_control_sheet.length;

    $(".js-gm-control-save-message").text(
      "You are saving " + number_items + " characters in this group",
    );
    ...
    $(".js-gm-control-save-dialog-action-button").click(function () {
      ...
      else export_json = gm_control_export_json(false);
      ...
      local_storage_save(
        "gm_control_saved_items",
        export_json,
        false,
        save_name,
      );
    });
    if (gm_control_sheet.length > 0) $(".js-gm-control-save-dialog").modal();
    else
      create_alert(
        "You need to have items in your control sheet to save them",
        "warning",
      );
    return false;
  });
  ...
  $(".js-gm-control-load").click(function (event) {
    ...
    $(".js-gm-control-load-dialog-load").click(function (event) {
      ...
      do_it = false;
      if (
        gm_control_sheet.length > 0 &&
        $(".js-gm-control-load-dialog-overwrite").is(":checked")
      ) {
        if (
          confirm(
            "Are you sure you want to overwrite the NPCs in your current control group?",
          )
        )
          do_it = true;
      } else {
      ...
      if (do_it) {
        ...
        loaded_items = local_storage_retrieve(
          "gm_control_saved_items",
          item_index,
        );
  ...
  function propogate_load_items() {
    ...
    if (saved_items.length > 0) {
      load_html +=
        '<label><input type="checkbox" name="overwrite" class="js-gm-control-load-dialog-overwrite" /> Overwrite NPCs</label>';
      load_html += "<table>";
      load_html += "<tr>";
      ...
      for (
        saved_item_count = 0;
        saved_item_count < saved_items.length;
        saved_item_count++
      ) {
        ...
        load_html +=
          ' <a href="#" ref="' +
          saved_item_count +
          '" title="Load This Group" class="js-gm-control-load-dialog-load"><span class="glyphicon glyphicon-floppy-open"></span></a> ';
        load_html +=
          ' <a href="#" ref="' +
          saved_item_count +
          '" title="Remove This Group" class="js-gm-control-load-dialog-remove"><span class="glyphicon glyphicon-trash"></span></a> ';
  ...
  $(".js-gm-control-export").click(function (event) {
    ...
    if (gm_control_sheet.length > 0) $(".js-gm-control-export-dialog").modal();
    else
      create_alert(
        "You need to have items in your control sheet to export them",
        "warning",
      );
  ...
  $(".js-gm-control-line-duplicate").click(function (event) {
    ...
    event.preventDefault();
    document.getSelection().removeAllRanges();
    gm_control_show_duplicate_line_dialog(
      gm_control_sheet[$(this).attr("ref")],
    );
    return false;
  });
  ...
  $(".js-gm-control-line-edit").click(function (event) {
    ...
    event.preventDefault();
    document.getSelection().removeAllRanges();
    gm_control_show_edit_line_dialog(
      gm_control_sheet[$(this).attr("ref")],
      $(this).attr("ref"),
    );
    return false;
  ...
  $(".js-gm-control-damage-modify").click(function (event) {
    ...
    if (
      $(this).next(".js-gm-control-damage-modify-dropdown").is(":visible") ==
      false
    ) {
  ...
  $(".js-gm-control-damage-modify-plus").click(function (event) {
    ...
    current_value =
      gm_control_sheet[current_selected].get_secondary("curr_hp") / 1;
    current_value++;
    ...
    if (gm_control_current_turn > 0) {
      ...
      $(".js-shock-damage-" + current_selected).text(
        "-" + gm_control_sheet[current_selected].shock_amount,
      );
      if (gm_control_sheet[current_selected].shock_amount > 0) {
  ...
  $(".js-gm-control-fatigue-modify").click(function (event) {
    ...
    $(".js-gm-control-damage-modify-dropdown").slideUp();
    if (
      $(this).next(".js-gm-control-fatigue-modify-dropdown").is(":visible") ==
      false
    ) {
      $(".js-gm-control-fatigue-modify-dropdown").slideUp();
  ...
  $(".js-gm-control-fatigue-modify-plus").click(function (event) {
    ...
    current_selected = $(this).attr("ref") / 1;
    current_value =
      gm_control_sheet[current_selected].get_secondary("curr_fatigue") / 1;
    current_value++;
    gm_control_sheet[current_selected].set_secondary(
      "curr_fatigue",
      current_value,
    );
  ...
  $(".js-gm-control-fatigue-modify-max").click(function (event) {
    ...
    current_value =
      gm_control_sheet[current_selected].get_secondary("max_fatigue") / 1;
  ...
  $(".js-gm-control-trash").click(function (event) {
    ...
    if (gm_control_sheet_currently_selected.length == 0) {
      create_alert(
        "Please select the characters you want to remove by placing a check next to their name.",
        "warning",
      );
    } else {
      if (confirm("Are you sure you want to remove the selected NPCs?")) {
        for (
          var i = gm_control_sheet_currently_selected.length - 1;
          i >= 0;
          i--
        )
...
function gm_control_sort_by_base_speed() {
  ...
  for (
    gm_rand_count = 0;
    gm_rand_count < gm_control_sheet.length;
    gm_rand_count++
  ) {
...
$(document).ready(function () {
  gm_control_current_sheet_obj = local_storage_retrieve(
    "gm_control_current_sheet",
    0,
  );
